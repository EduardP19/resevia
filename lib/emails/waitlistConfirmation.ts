// Branded confirmation email sent when a salon reserves their founding-member
// spot via the waitlist form. Returns subject + HTML + plain-text so the send
// can include a text/plain alternative (improves inbox placement).
...
export function buildWaitlistConfirmationEmail(firstName?: string): WaitlistConfirmationEmail {
  const name = (firstName || '').trim() || 'there';
  const subject = 'Your Resevia founding spot is confirmed 🎉';
  const preheader =
...
2. We set everything up for you — your AI receptionist is trained on your services, prices and opening hours.
...
© 2026 Resevia. London, United Kingdom — https://resevia.co.uk`;
